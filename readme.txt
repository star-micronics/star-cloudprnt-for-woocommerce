...
= 2.1.0 =
* Add many filters to allow the print job to be extended and customised by plugin/theme developers.
* Extend the document builder API for developers, so include fant B support, track font scaling, have built-in word-wrapping.
* Track print width reported by printers (refreshes eery 120s), instead of using built-in values.
* Enable printing order fees.
* planned: Plugin is now translatable
* planned: Configurable tax printing options (print line items with/without tax, per-rate tax totals, print total tax)
